...
void insere_aluno(s_aluno **pplista, s_aluno *no){ //insere struct ordenado
    s_aluno *aux = *pplista;
    s_aluno *anterior = NULL;

    if(aux == NULL){ //caso ele seja o primeiro
        *pplista = no;
        no->prox = NULL;
    }
    else{
        while(aux != NULL && aux->ra < no->ra){ //aqui é pra ele se localizar
            anterior = aux;
            aux = aux->prox;
        }
        if(aux == NULL){ //se ele cai aqui, insere no final
            anterior->prox = no;
            no->prox = NULL;
